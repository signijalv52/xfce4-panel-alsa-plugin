...
//============================================================================================
static void sliderChanged(GtkRange* range, mixerStruct* mixer)
{
  long volume;
  ...
  if (volume > 0)
  {
    if (snd_mixer_selem_has_playback_switch(mixer->channel))
    {
      snd_mixer_selem_set_playback_switch(mixer->channel, SND_MIXER_SCHN_FRONT_LEFT, 1);
      snd_mixer_selem_set_playback_switch(mixer->channel, SND_MIXER_SCHN_FRONT_RIGHT, 1);
    }
  }
  else
  {
    if (snd_mixer_selem_has_playback_switch(mixer->channel))
    {
      snd_mixer_selem_set_playback_switch(mixer->channel, SND_MIXER_SCHN_FRONT_LEFT, 0);
      snd_mixer_selem_set_playback_switch(mixer->channel, SND_MIXER_SCHN_FRONT_RIGHT, 0);
    }
  }
...
static void pluginConstruct(XfcePanelPlugin* rootWidget)
{
  ...
  widgets->connection = g_signal_connect(widgets->slider, "value-changed", G_CALLBACK(sliderChanged), widgets->mixer);
